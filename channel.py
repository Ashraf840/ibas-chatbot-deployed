...
class CollectingOutputChannel(OutputChannel):
    ...
    #############################
    # async def find_match(self, text, dataset_path, column_name):
    #     workbook = openpyxl.load_workbook(dataset_path)
    #     sheet = workbook['Sheet1']

    #     for row in sheet.iter_rows(min_row=2, values_only=True):
    #         answer = row[sheet[column_name].index]
    #         if text == answer:
    #             return 1
        
    #     return 0
    async def text_similarity(self, a, b):
        return SequenceMatcher(None, a, b).ratio()
    
    async def find_match(self, text, data_df, column_name, similarity_threshold):
        # answer_column = data_df[column_name]
        ...
        for answer in answer_column:
            if isinstance(answer, str):
                sim_score = await self.text_similarity(text, answer)
    ...
    async def send_text_message(
        ...
        # text_to_match = "something"
        dataset_path = "/home/robin/Documents/Chat_operator/test_chatbot_active/ibas-chatbot-demo/data/IBAS_data_en_bn_same_line.xlsx"
        sheet_name = "Sheet1"
        answer_column_name = "Answer"
        similarity_threshold = 0.5
        # result = await self.find_match(text_to_match, dataset_path)
        data_df = pd.read_excel(dataset_path, sheet_name=sheet_name)
        for message_part in text.strip().split("\n\n"):
            # result = await self.find_match(message_part, dataset_path, answer_column_name)
            result = await self.find_match(message_part, data_df, answer_column_name, similarity_threshold)
            predicted = result
            await self._persist_message(self._message(recipient_id, text=message_part, predicted = predicted))

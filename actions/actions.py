...
class ActionCreateIssue(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        response = requests.get(f'http://172.16.6.91/home/api/user-chatbot/socket/{sender_id}/')
        data = response.json()
        # return data
        ...
        if response['code'] == 200:
            ...
            url = "http://172.16.6.91/home/api/user-chatroom/socket/"
	    ...
	    "user_organization": f"{data['user_organization']}",
            ...
            print("Inside if after the response 200")

            # response = requests.request("POST", url, headers=headers, data=payload)
            response = requests.post(
                "http://172.16.6.91/home/api/user-chatroom/socket/", payload,
                headers= headers
            )
